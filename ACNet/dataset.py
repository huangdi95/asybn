...
MNIST_PATH = '/lustre/S/huangdi/datasets/torch_mnist/'
CIFAR10_PATH = '/lustre/S/huangdi/datasets/cifar-10-batches-py/'
CH_PATH = '/lustre/S/huangdi/datasets/torch_ch/'
SVHN_PATH = '/lustre/S/huangdi/datasetstorch_svhn/'
...
def create_dataset(dataset_name, subset, batch_size):
    ...
    #   copied from https://github.com/pytorch/examples/blob/master/mnist/main.py
        ...
        if subset == 'train':
            ...
            return InfiniteDataLoader(datasets.CIFAR10(CIFAR10_PATH, train=True,
                                transform=transforms.Compose([
                                    transforms.ToTensor(),
                                    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])),
                                batch_size=batch_size, shuffle=False)
